...
create table if not exists ai_training_rules (
  ...
  notes text,
  constraint ai_training_rules_category_check check (category in (
    'tone',
    'services',
    'service_area',
    'pricing',
  ...
  constraint ai_training_rules_visibility_check check (visibility in ('public', 'internal')),
  constraint ai_training_rules_status_check check (status in ('draft', 'approved', 'live', 'archived'))
);

create table if not exists ai_helper_versions (
  id uuid primary key default gen_random_uuid(),
  created_at timestamptz not null default now(),
...
alter table ai_helper_versions enable row level security;
...
drop policy if exists "owner manage ai training rules" on ai_training_rules;
drop policy if exists "owner manage ai helper versions" on ai_helper_versions;
...
create policy "owner manage ai training rules" on ai_training_rules for all
  using ((auth.jwt() ->> 'email') = 'team@urbanyards.us')
  with check ((auth.jwt() ->> 'email') = 'team@urbanyards.us');

create policy "owner manage ai helper versions" on ai_helper_versions for all
  using ((auth.jwt() ->> 'email') = 'team@urbanyards.us')
  with check ((auth.jwt() ->> 'email') = 'team@urbanyards.us');
...
create index if not exists ai_training_rules_status_visibility_idx on ai_training_rules(status, visibility);
create index if not exists ai_training_rules_category_idx on ai_training_rules(category);
create index if not exists ai_training_rules_priority_idx on ai_training_rules(priority, updated_at desc);
create index if not exists ai_helper_versions_published_idx on ai_helper_versions(published_at desc);
...
insert into ai_training_rules (title, category, content, visibility, status, priority, published_at) values
  ('Friendly grounded tone', 'tone', 'Use friendly, grounded, owner-operated language. Avoid pushy sales wording, generic corporate phrasing, or exaggerated promises.', 'public', 'live', 10, now()),
  ('Pricing and estimate boundary', 'pricing', 'Do not guarantee pricing or give final estimates. Explain that pricing depends on property size, condition, access, frequency, and scope, then invite the visitor to request a free quote or walkthrough.', 'public', 'live', 20, now()),
  ('Service area boundary', 'service_area', 'When asked about service area, say Urban Yards serves Portland, Vancouver & Beaverton. If the visitor is nearby but unsure, suggest using the quote form or contacting Urban Yards.', 'public', 'live', 30, now()),
  ('Licensed work boundary', 'do_dont', 'Do not claim licensed trade work, legal guarantees, insurance promises, or regulated contractor work unless Urban Yards has explicitly approved that language.', 'public', 'live', 40, now()),
  ('Lead capture guidance', 'lead_capture', 'When a visitor is ready for help, guide them toward Request a Free Quote and ask for practical details: name, property address or area, service needed, timing, photos if available, and contact info.', 'public', 'live', 50, now())
on conflict do nothing;
```
