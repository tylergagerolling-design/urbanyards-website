...
test("archived workspace polish remains available with current dashboard assets", () => {
  ...
  assert.match(html, /dashboard\.css\?v=20260908-visual-consistency-1/);
  assert.match(html, /dashboard\.js\?v=20260908-visual-consistency-1/);
});
...
test("Tickets Leads and Money use the same flat white page canvas as Home and Work", () => {
  ...
  assert.match(html, /dashboard-unified\.css\?v=20260908-visual-consistency-1/);
...
test("Tickets and Work filter text clears the embedded icons", () => {
  const css = read("dashboard-unified.css");
  const html = read("dashboard.html");
...
test("Route Planner uses the flat full-width dashboard canvas", () => {
  const css = read("dashboard-unified.css");
  assert.match(css, /> #route-planner\.dashboard-section\.is-active \{[\s\S]*?width:100%!important;[\s\S]*?max-width:none!important;[\s\S]*?padding:0!important;[\s\S]*?background:#fff!important;/);
  assert.match(css, /#route-planner > \[data-route-week-planner\],[\s\S]*?width:100%!important;[\s\S]*?max-width:none!important;[\s\S]*?padding:0!important;[\s\S]*?background:#fff!important;/);
  assert.match(css, /#route-planner \.route-week-cards \{[\s\S]*?width:100%!important;[\s\S]*?max-width:none!important;/);
});
...
test("separate AI products keep the dashboard Keaton Mask launcher and distinct public identity", () => {
  ...
  const monochromeCss = css.slice(css.indexOf("The Lawnmower Man — monochrome Keaton Mask popup"));
...
test("The Lawnmower Man exposes the selected owner operations suite with review safeguards", () => {
  const js = read("dashboard.js");
  const css = read("dashboard.css");
  const api = read("api/groundskeeper-ai.js");
...
test("Unified Ticket edits the canonical record, syncs its work order, and verifies the Street View address", () => {
  const js = read("dashboard.js");
  const css = read("dashboard.css");

  assert.match(js, /function renderUnifiedTicketEditForm/);
  assert.match(js, /data-unified-ticket-edit-form/);
  assert.match(js, /name="property_address"[^>]*data-address-autocomplete/);
  assert.match(js, /data-action="unified-ticket-verify-address"/);
  assert.match(js, /Use the ticket workflow controls to change lifecycle status/);
  assert.match(js, /geocodeRouteAddress\(address\)/);
  assert.match(js, /const savedTicket = await updateJobTicket\(ticketId, ticketPayload\)/);
  assert.match(js, /const savedJob = await updateScheduledJob\(linkedJobId/);
  assert.match(js, /Ticket and linked work order updated/);
  assert.match(js, /function loadUnifiedTicketStreetView/);
...
test("Call Queue omits the retired Google Voice panel and preserves the entry-table workspace", () => {
  ...
  assert.match(js, /Manage callers, follow-ups, and qualified leads/);
