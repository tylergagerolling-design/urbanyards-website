...
function Get-UyPetSessionEntropy {
    return [Text.Encoding]::UTF8.GetBytes("UrbanYardsPet.QuoteNotifications.v1")
}
...
function Save-UyPetAuthSession {
    ...
    $plain = $null
    try {
        $plain = [Text.Encoding]::UTF8.GetBytes(($Session | ConvertTo-Json -Depth 6 -Compress))
        ...
        Write-UyPetLog "Encrypted dashboard session saved for the current Windows user."
    }
    finally {
...
function Get-UyPetAuthSession {
    ...
    try {
        ...
        Write-UyPetLog "The saved dashboard session could not be read and was discarded." "WARN"
...
function Remove-UyPetAuthSession {
    ...
    Write-UyPetLog "Dashboard quote notifications disconnected."
}

function Get-UySupabasePublicConfig {
    param([Parameter(Mandatory = $true)]$Config)
    $dashboardUri = [Uri]([string]$Config.dashboardUrl)
    if ($dashboardUri.Scheme -ne "https") { throw "The dashboard address must use HTTPS." }
    $site = $dashboardUri.GetLeftPart([UriPartial]::Authority)
...
function Connect-UyPetToUrbanYards {
    ...
    try {
        ...
        Write-UyPetLog "Urban Yards dashboard connection succeeded."
        return $session
    }
    catch {
        Write-UyPetLog "Urban Yards dashboard sign-in failed." "WARN"
...
function Get-UyUsableAccessToken {
    ...
    try {
        ...
        Write-UyPetLog "The dashboard session expired and could not be refreshed." "WARN"
        Remove-UyPetAuthSession
        return ""
    }
}
...
function Get-UyPetConnectedEmail {
    $session = Get-UyPetAuthSession
    if ($session) { return [string]$session.email }
    return ""
}
